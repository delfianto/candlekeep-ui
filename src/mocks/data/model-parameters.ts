export const modelFamiliesParameterDocs = {
  ...
  seed: {
    ...
    detailed_info:
      "If set, the AI will generate the exact same response every time (assuming other settings match). Useful for testing prompt changes or debugging a broken character card.",
  },
  frequency_penalty: {
  ...
  max_completion_tokens: {
    ...
    detailed_info:
      "The maximum text generation allowed. Lower this if you want snappy dialogue or short emotes. Raise this for slow-burn, novella-style responses. (Note: This matches OpenAI's 'max_completion_tokens').",
  },
  max_tokens: {
    label: "Max Response Length",
    short_info: "Hard limit on how much the character can write.",
    detailed_info:
      "The maximum text generation allowed. Set higher for 'Purple Prose' or detailed scene descriptions. (Matches Anthropic's naming convention).",
  },
  max_output_tokens: {
    label: "Max Response Length",
    short_info: "Hard limit on how much the character can write.",
  ...
  reasoning_effort: {
    ...
    detailed_info:
      "Controls the depth of the internal thought chain. 'High' is useful for complex lore checks, solving riddles, or political intrigue RPs where keeping facts straight is critical. 'Low' is faster and better for casual chat.",
  },
  thinking: {
    label: "Thinking Budget (Claude)",
    short_info: "Enable internal monologue before speaking.",
    detailed_info:
      "Allows the character to 'think' silently before replying. Incredible for RP: the character can plan lies, weigh emotional reactions, or recall obscure lore without polluting the chat log. Set a budget (e.g., 2000 tokens) to give them space to ponder.",
  },
  logit_bias: {
    label: "Logit Bias (Banned/Forced Words)",
    short_info: "Manually ban or force specific words.",
    detailed_info:
      "Advanced: Allows you to mathematically prevent the model from using certain words (e.g., modern slang in a fantasy RP) or drastically increase the chance of specific formatting.",
  },
  system: {
    label: "System Prompt Override",
    short_info: "The core instructions defining the world and character.",
    detailed_info:
      "Usually handled automatically by your Character Card. Overrides the base instructions. Use this to enforce global rules like 'Write in third person past tense' or 'NSFW allowed'.",
  },
  developer_message: {
    label: "Developer Message (O1/GPT-5)",
    short_info: "The foundational rules for reasoning models.",
    detailed_info:
      "Similar to System Prompt, but treated with higher priority by O1/GPT-5 class models. Use this for strict jailbreaks or absolute formatting rules.",
  },
  safety_settings: {
    label: "Safety Filters",
    short_info: "Content filtering sensitivity.",
    detailed_info:
      "Controls how aggressive the provider is about blocking NSFW or violent content. For RP, this usually needs to be minimized or disabled if possible.",
  },
  response_format: {
  ...
  n: {
    ...
    detailed_info:
      "Create multiple drafts for the same message so you can pick the best one. Increases cost/time.",
  },
  stream: {
    label: "Stream Response",
    short_info: "Typewriter effect.",
    detailed_info:
      "If enabled, you see the text appearing as it is written. If disabled, you wait until the whole message is done.",
  },
};
